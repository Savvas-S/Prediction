#!/usr/bin/env python3
"""
fit_model_find_last10.py (FAST, GPU train + cached features) + partial saves
...
- Saves EVERY trial that reaches >= 4/10 as partial JSON:
    partials/trial{N}_{S}of10.json
  (S = number of steps found before failing; includes the list of matched draws)
...
p.add_argument("--max-trials", type=int, default=1000, help="Max random configs to try.")
...
p.add_argument("--config", default=None, help="Path to a JSON config to run directly (no random search).")
...
# Filter rows that aren't exactly 6 ints
...
def save_partial(trial_num: int, steps_ok: int, found_prefix: list, cfg: ModelConfig):
    safe_name = f"trial{trial_num}_{steps_ok}of10.json"
    outpath = partials_dir / safe_name
    payload = {
        "success_steps": steps_ok,
        "found_prefix": found_prefix,  # list of draws matched in order
        "config": asdict(cfg),
    }
...
# If a config was provided, run it directly (no random search loop)
if args.config:
    cfg = load_config(args.config)
    # clamp to caps (just in case file exceeds your runtime limits)
    ...
    if args.verbose:
        print(f"\n[CONFIG MODE] Using config from {args.config}:\n{json.dumps(asdict(cfg), indent=2)}")
    ...
    for idx, target in enumerate(held10, start=1):
        ...
        found_sets, _, _, _ = predict_sets_once(cache, cfg, t_end, seed_base=args.seed + 999*idx)
    ...
    if steps_ok >= 4:
        save_partial(trial_num=0, steps_ok=steps_ok, found_prefix=found_prefix, cfg=cfg)

    if ok:
        winning_cfg = cfg
        print("\nSUCCESS: Provided config hits all 10 in sequence.")
    ...
    for trial in range(1, args.max_trials + 1):
        ...
        steps_ok = len(found_prefix)
        if steps_ok >= 4:
            save_partial(trial_num=trial, steps_ok=steps_ok, found_prefix=found_prefix, cfg=cfg)
...
if winning_cfg is None:
    ...
    # still exit cleanly; partials are saved already
    raise SystemExit(0)
