...
# CLI
# ----------------------
parser = argparse.ArgumentParser(description="JOKER ML predictor (RandomForest probabilities + diversity).")
parser.add_argument("--csv", default="joker_results2_reversed.csv", help="CSV with draws, header + 6 cols (5 mains + Joker).")
...
parser.add_argument("--exclude-main", type=int, nargs="*", default=[], help="Excluded main numbers.")
parser.add_argument("--exclude-joker", type=int, nargs="*", default=[], help="Excluded joker numbers.")
...
# ----------------------
# Load CSV (already oldest → newest from scraper)
# ----------------------
rows = []
...
def weighted_sample_without_replacement(items, pmap, k):
    keys = [(math.log(random.random()) / pmap.get(x, 0.000001), x) for x in items]
    keys.sort()
...
top8 = sorted([n for n, _ in sorted(id_to_p_main.items(), key=lambda kv: kv[1], reverse=True) if n not in exclude_main][:8])
